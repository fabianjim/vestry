...
public class DemoSessionServiceTest {
    ...
    void createSessionDeduplicatesTagsByNameAcrossEntries() {
        me.vestry.model.Tag shared = new me.vestry.model.Tag();
        shared.setId(50);
        shared.setName("win");
        ...
        me.vestry.model.Tag firstTag = session.getJournalEntries().get(0).getTags().iterator().next();
        me.vestry.model.Tag secondTag = session.getJournalEntries().get(1).getTags().iterator().next();
        assertSame(firstTag, secondTag);

        List<me.vestry.model.Tag> popular = demoSessionService.getPopularTags(session, "", 10);
    ...
    void getPnLSummary_PartialSaleThenPurchase() {
        ...
        DemoSession session = new DemoSession();
        session.setTransactions(history);
        Stock latest = new Stock();
        latest.setCurrentPrice(160);
        when(stockService.getLatestStockData("AAPL")).thenReturn(Optional.of(latest));
        PnLSummaryDTO result = demoSessionService.getPnLSummary(session);
